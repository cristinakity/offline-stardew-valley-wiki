...
function copyPages(source, target, selectedLanguages, visit, control) {
  for (const entry of fs.readdirSync(source, { withFileTypes: true })) {
    control?.();
    const sourcePath = path.join(source, entry.name);
    const targetPath = path.join(target, entry.name);
    if (entry.isDirectory()) copyPages(sourcePath, targetPath, selectedLanguages, visit, control);
    if (entry.isFile()) {
...
export function prepareEdition(sourceArgument, targetArgument, languageArgument, options = {}) {
  ...
  for (const language of languages) {
    copyPages(path.join(source, language, 'pages'), path.join(target, language, 'pages'), selected, contents => {
      ...
      options.onProgress?.({ phase: 'selecting', pages: pageCount, assets: 0 });
    }, options.control);
  ...
  while (pendingAssets.size) {
    ...
    options.onProgress?.({ phase: 'selecting', pages: pageCount, assets: assetCount, assetBytes });
  ...
  const manifest = { schema: 1, edition, languages, pages: pageCount, assets: assetCount, asset_bytes: assetBytes };
  ...
  options.onProgress?.({ phase: 'validating', ...manifest });
  return manifest;
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    const manifest = prepareEdition(...process.argv.slice(2));
