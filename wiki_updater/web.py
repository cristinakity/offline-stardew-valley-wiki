...
async def make_candidate(request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        run_id = int(requested_run_id) if requested_run_id is not None else None
        return create_candidate(
            settings,
            db,
            str(payload.get("version", "v2.0.0")),
            actor(request),
            run_id=run_id,
        )
...
async def purge_storage(request: Request) -> dict[str, Any]:
    ...
    if db.one("SELECT id FROM build_jobs WHERE status IN ('queued','building') LIMIT 1"):
        raise HTTPException(409, "Storage cannot be purged while a build is queued or running.")
    payload = await request.json()
...
<div class="help-item"><h3>Create candidate</h3><p>Congela el snapshot de un run terminado en una versión, genera manifiesto, checksums y un <code>.tar.zst</code>. Un <b>sample</b> puede convertirse en candidato para probar builds, pero sólo contiene unas 25 páginas por idioma y no debe publicarse como release final.</p></div>
...
<section class="card"><h2>Create candidate</h2><div class="row"><input id="version" value="v2.0.0" pattern="v?[0-9]+\\.[0-9]+\\.[0-9]+(?:-[0-9A-Za-z.-]+)?"><button onclick="createCandidate()">Create from current snapshot</button></div><p><small>La versión debe ser única. Para un sample usa, por ejemplo, <code>v2.0.0-sample.21</code>, o créalo directamente desde Crawler/Runs.</small></p></section></div>
</section>
...
<section id="tab-builds" class="tab-panel" data-panel="builds"><section class="card"><div class="section-heading"><div><h2>Builds locales</h2><small>Preversiones Linux generadas desde el snapshot aprobado. No se publican en GitHub automáticamente. Durante un build, pulsa <b>Refresh</b> para leer el progreso más reciente sin recargar continuamente.</small></div><span class="muted">Más reciente primero</span></div><div id="builds" class="list-scroll"><small>Cargando builds…</small></div></section></section>
...
async function createCandidateFromRun(id,profile){const suggested=`v2.0.0-${profile}.${id}`;const version=prompt(`Crear candidato desde run #${id} (${profile}). La versión debe ser única.${profile==='sample'?'\\n\\nEste sample es sólo para pruebas: no contiene la wiki completa.':''}`,suggested);if(!version)return;try{await api('/api/candidates',{method:'POST',body:JSON.stringify({version,run_id:id})});showTab('candidates')}catch(e){alert(e)}}
