...
function showTab(name){if(!validTabs.has(name))name='overview';document.querySelectorAll('[data-panel]').forEach(panel=>panel.classList.toggle('active',panel.dataset.panel===name));document.querySelectorAll('[data-tab]').forEach(button=>button.setAttribute('aria-selected',String(button.dataset.tab===name)));localStorage.setItem('updater.activeTab',name);window.scrollTo({top:0,behavior:'smooth'})}
...
async function refresh(){try{const [s,r,c,conf,audit,space]=await Promise.all([api('/api/status'),api('/api/runs'),api('/api/candidates'),api('/api/settings'),api('/api/audit?limit=25'),api('/api/storage')]);
document.querySelector('#status').innerHTML=`Environment: <b>${s.environment}</b><br>Storage real: ${bytes(s.storage_used_bytes)} / ${bytes(s.storage_limit_bytes)}<br>Snapshot: ${s.current?.snapshot_id||'none'}<br>Active: ${s.active_run?.id||'none'}`;
document.querySelector('#storageSummary').innerHTML=`Uso físico real: <b>${compactBytes(space.physical_bytes)}</b> de ${bytes(s.storage_limit_bytes)}`;document.querySelector('#storageBreakdown').innerHTML=`<table><thead><tr><th>Categoría</th><th>Espacio asignado</th><th>Archivos</th></tr></thead><tbody>${Object.entries(space.categories).map(([name,value])=>`<tr><td>${name}</td><td>${compactBytes(value.allocated_bytes)}</td><td>${value.files}</td></tr>`).join('')}</tbody></table>`;
...
document.querySelector('#runs').innerHTML=r.map(x=>`<tr data-run-id="${x.id}" class="${selectedRunId===x.id?'selected-run':''}"><td><a class="run-link" aria-current="${selectedRunId===x.id?'true':'false'}" href="#" onclick="detail(${x.id});return false">${x.id}</a></td><td>${x.profile}</td><td>${x.status}</td><td>${x.created_at}</td><td>${x.snapshot_id||''}</td><td>${['queued','running'].includes(x.status)?`<button class="danger" onclick="cancelRun(${x.id})">Cancel</button>`:x.recoverable?`<button class="secondary" onclick="recoverRun(${x.id})">Recover</button>`:''}</td></tr>`).join('');
...
const activityId=selectedRunId||s.active_run?.id||r[0]?.id;if(activityId){const activity=await api(`/api/runs/${activityId}`);if(selectedRunId===null||selectedRunId===activityId)renderActivity(activity)}
...
async function recoverRun(id){if(!confirm(`¿Recuperar el run #${id} desde los blobs locales? No volverá a descargar toda la wiki.`))return;try{const job=await api(`/api/runs/${id}/recover`,{method:'POST'});selectedRunId=job.run_id;await refresh();showTab('crawler')}catch(e){alert(e)}}
...
async function detail(id){selectedRunId=id;markSelectedRun();showTab('crawler');document.querySelector('#activityTitle').textContent=`Run #${id} · cargando historial…`;document.querySelector('#activityState').textContent='loading';document.querySelector('#activityState').className='pill';try{const activity=await api(`/api/runs/${id}`);if(selectedRunId===id)renderActivity(activity)}catch(e){if(selectedRunId===id){document.querySelector('#activityState').textContent='error';document.querySelector('#activityState').className='pill bad';document.querySelector('#activity').innerHTML=`<p class="bad">No se pudo cargar el run #${id}: ${e}</p>`}}}
...
showTab(localStorage.getItem('updater.activeTab')||'overview');showProfileHelp();refresh();setInterval(refresh,3000);
